...
export default function ServiceConfigurator({
  ...
  if (selectedServices.length === 0) {
    return (
      <div className="bg-white/80 dark:bg-black/60 backdrop-blur-md rounded-2xl p-12 border border-slate-200/80 dark:border-white/10 text-center">
    ...
    <div className="space-y-5 animate-fadeIn">
      {selectedServices.map((srv, idx) => {
        ...
        return (
          <div
            key={srv.key}
            className={`group/card bg-white/90 dark:bg-black/60 backdrop-blur-md rounded-2xl border shadow-sm overflow-hidden transition-all duration-300 ${
              isOpen
                ? "border-amber-200 dark:border-amber-900/50 shadow-lg shadow-amber-500/5"
                : "border-slate-200/80 dark:border-white/10 hover:shadow-md hover:border-slate-300 dark:hover:border-white/20"
            }`}
          >
            ...
            <button
              ...
              className="w-full flex items-center justify-between gap-4 p-5 sm:p-6 text-left hover:bg-slate-50/80 dark:hover:bg-white/5 transition-colors duration-200"
            >
              <div className="flex items-center gap-4 min-w-0">
                <div
                  className="flex h-10 w-10 items-center justify-center rounded-xl text-black font-bold text-sm shrink-0 shadow-sm shadow-amber-500/30 transition-transform duration-300 group-hover/card:scale-105"
                ...
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 shrink-0">
              ...
              <div className="flex items-center gap-3 sm:gap-5 shrink-0">
                ...
                <div
                  className={`p-1.5 rounded-lg transition-all duration-300 ${
                    isOpen
                      ? "bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-300 rotate-180"
            ...
            {/* Expanded Body */}
            {isOpen && (
              <div className="px-5 pb-6 sm:px-6 sm:pb-8 border-t border-slate-200/60 dark:border-white/10 pt-6 space-y-8 animate-fadeIn">
                {/* 1. Select Tier */}
                <div>
                      ...
                      return (
                        <button
                          ...
                          className={`group relative flex flex-col justify-between min-h-[136px] p-4 sm:p-5 rounded-xl border text-left transition-all duration-200 hover:-translate-y-0.5 active:translate-y-0 ${
                            isSelected
                              ? "border-amber-500 bg-amber-50/90 dark:bg-amber-950/20 shadow-md hover:shadow-lg ring-2 ring-amber-500/20"
                              : "border-slate-200 dark:border-white/10 bg-white dark:bg-black/40 hover:border-amber-300 dark:hover:border-amber-700/50 hover:shadow-md"
                          }`}
                        >
                          <div className="min-w-0">
                            <div className="flex items-start justify-between gap-2">
                              <span className="font-bold text-slate-900 dark:text-white text-base leading-snug break-words transition-colors group-hover:text-amber-700 dark:group-hover:text-amber-300">
                                {t.name}
                              </span>
                              {isSelected && (
                                <span
                          ...
                          <div className="mt-4 pt-3 border-t border-slate-200/60 dark:border-white/10 flex items-baseline justify-between gap-2">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-4 max-w-sm">
                      <input
                        ...
                        className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-black/40 text-slate-900 dark:text-white font-bold transition-shadow duration-200 focus:outline-none focus:ring-2 focus:ring-amber-500 hover:border-amber-300 dark:hover:border-amber-700/50"
                      />
                        ...
                        return (
                          <button
                            ...
                            className={`px-4 py-2.5 rounded-xl border font-semibold text-sm transition-all duration-200 hover:-translate-y-0.5 active:translate-y-0 ${
                              isSelected
                                ? "border-transparent text-black shadow-md hover:shadow-lg hover:opacity-90"
                                : "border-slate-200 dark:border-white/10 bg-white dark:bg-black/40 text-slate-700 dark:text-slate-300 hover:border-amber-300 dark:hover:border-amber-700/50 hover:shadow-sm"
                            }`}
                            style={
                              isSelected
                          ...
                          <button
                            ...
                            className={`flex items-center justify-between gap-3 p-4 rounded-xl border text-left transition-all duration-200 hover:-translate-y-0.5 active:translate-y-0 ${
                              isChecked
                                ...
                                : "border-slate-200 dark:border-white/10 bg-white dark:bg-black/40 text-slate-700 dark:text-slate-300 hover:border-slate-300 dark:hover:border-white/20 hover:shadow-sm"
                            }`}
                          >
                              ...
                              <span
                                className={`flex h-4 w-4 items-center justify-center rounded-full shrink-0 transition-colors duration-200 ${
                                  isChecked
                                    ...
                                    : "bg-slate-100 dark:bg-white/10 text-transparent"
                                }`}
                              >
                                <Check className="h-3 w-3 stroke-[3]" />
                              </span>
                            ...
                            <span className="text-xs font-bold px-2 py-1 rounded-md bg-slate-100 dark:bg-white/10 text-slate-800 dark:text-slate-200 shrink-0 tabular-nums">
